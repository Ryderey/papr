...
- 移动端只允许一个 Profile 处于生效状态，摘要与全文翻译均使用它。启用某项时 Core 在同一次设置写入中关闭其余项；关闭当前项时不选择备用项，因此可以没有生效的 AI 配置，此时摘要页和翻译页只提示用户先启用配置而不发起网络请求。读取旧版多生效配置时，Core 按旧摘要选择顺序保留一个并持久化关闭其余项。
...
- Android 以 `credential_ref` 为 Keystore alias。Flutter 仅在发起连接测试、摘要或 LLM 翻译的瞬间经平台通道读取密钥，并将其作为非持久化参数传入 FRB；Core 只在该请求生命周期内持有它，绝不写 SQLite、change_log、缓存、日志或错误 detail。移动端不接入 Google、DeepL、Bing 的官方或非官方翻译接口。
...
- 稳定错误码为 `invalidAiProfile`、`noAiCredential`、`noArticleBody`、`emptyAiQuestion`、`aiAuth`、`aiRateLimited`、`aiNetwork`、`aiParse`、`aiNoVisibleOutput`、`aiCancelled`、`aiRequestNotFound`；Flutter 只本地化 stable code，且不展示 provider 原始响应体。
- SenseNova 6.8 Flash Lite 的 `reasoning` 会计入 `max_tokens`，但阅读器只展示 `content`。Core 为这个已知模型使用 8,192 的输出上限，其他 Profile 保持通用的 2,000 上限；流正常结束但累计 `content` 为空时返回 `aiNoVisibleOutput`，而非法 SSE 仍为 `aiParse`。
...
2. `papr-core::translate` owns HTML block chunking and non-translatable-node preservation. It delegates LLM batches to `ai`; it reports batch-level progress, never token-level progress for a full article.
